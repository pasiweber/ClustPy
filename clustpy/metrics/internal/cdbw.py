...
def rep(n_clusters, dimension, n_points_in_cl, coord_in_cl, labels_in_cl):
    ...
    for i in range(n_clusters):
        ...
        if n_points_in_cl[i] >= 4:
            if (
                len(np.unique(coord_in_cl[i, 0 : n_points_in_cl[i], 0:dimension][:, 0])) >= 4
                and coord_in_cl[i, 0 : n_points_in_cl[i], 0:dimension].shape[1] > dimension
            ):
    ...
    for i in range(n_clusters):
        for j in range(n_clusters):
            if i > j:
                ...
                for u, v in cl_r[(i, j)]:
                    middle_point[(i, j)].append(
                        (X[rep_in_cl[i, 0 : n_rep[i]][u]] + X[rep_in_cl[j, 0 : n_rep[j]][v]]) / 2
                    )
...
def art_rep(X, n_clusters, rep_in_cl, n_rep, n_rep_max, mean_arr, s, dimension):
    ...
    for i in range(n_clusters):
        ...
        for k in range(0, s):
            a_rep_shell[i, k, 0 : n_rep[i], 0:dimension] = (1 - (k / s)) * X[rep_in_cl[i, 0 : n_rep[i]]] + (
                k / s
            ) * mean_arr[i]
    return a_rep_shell
...
def compactness(n_clusters, stdev, a_rep_shell, n_rep, n_points_in_cl, s, coord_in_cl, n_max, n_rep_max, metric):
    ...
    for i in range(0, n_clusters):
        for k in range(0, s):
            dist[i, k, 0 : n_points_in_cl[i], 0 : n_rep[i]] = cdist(
                coord_in_cl[i, 0 : n_points_in_cl[i], :], a_rep_shell[i, k, 0 : n_rep[i], :], metric=metric
            )
        ...
        for j in range(n_clusters):
            if i > j:
                ...
                coord_f = np.vstack(
                    (
                        coord_f,
                        np.vstack((coord_in_cl[i, 0 : n_points_in_cl[i], :], coord_in_cl[j, 0 : n_points_in_cl[j], :])),
                    )
                )
...
def cdbw_score(X, labels, metric="euclidean", noise_strategy="filter", intra_dens_inf=False, s=10, multipliers=False) -> np.ndarray:
    """
    ...
    try:
        ...
        return 0
    compact, cohesion = compactness(
        n_clusters, stdev, a_rep_shell, n_rep, n_points_in_cl, s, coord_in_cl, n_max, n_rep_max, metric
    )
